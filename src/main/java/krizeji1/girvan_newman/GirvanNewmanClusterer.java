...
import krizeji1.edge_betweenness.EdgeBetweenness;
import krizeji1.girvan_newman.multicolour.GraphColorizer;
...
public class GirvanNewmanClusterer implements Clusterer, LongTask {
    ...
    public void execute(GraphModel gm) {
        ...
        if (this.preferred > 0) {
            ...
            // SelectClustersPanel panel = new
            // SelectClustersPanel(getClusterCounts());
            //
            // DialogDescriptor dd = new DialogDescriptor(panel, "Select number
            // of clusters", true, null);
    ...
    private AttributeColumn copyClusterToPrev() {
        ...
        if (nodeTable.hasColumn(CLUSTER)) {
            ...
            AttributeColumn duplicateColumn = Lookup.getDefault().lookup(org.gephi.datalab.api.AttributeColumnsController.class).duplicateColumn(nodeTable, clusterColumn, PREV_CLUSTER,
                    AttributeType.INT);
