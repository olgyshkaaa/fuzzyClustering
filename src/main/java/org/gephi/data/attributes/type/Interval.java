...
public final class Interval<T> implements Comparable<Interval> {
     ...
     *
     * <p>Note that {@code value} cannot be null if you want use this {@code interval} as a value storage. If it is null some estimators could not work and generate exceptions.
     *
     ...
     * trichotomy}; that is, exactly one of the following three properties holds: <ol> <li> <i>i</i> and <i>i'</i> overlap;
     *
     * <li> <i>i</i> is to the left of <i>i'</i> (<i>i.high < i'.low</i>);
     *
     * <li> <i>i</i> is to the right of <i>i'</i> (<i>i'.high < i.low</i>). </ol>
     *
     * <p>Note that if two intervals are equal ({@code i.low = i'.low} and {@code i.high = i'.high}), they overlap as well. But if they simply overlap (for instance {@code i.low < i'.low} and {@code i.high >
     ...
     * @return a negative integer, zero, or a positive integer as this interval is to the left of, overlaps with, or is to the right of the specified interval.
     ...
     *
     * <p>Note that two intervals are equal if {@code i.low = i'.low} and {@code i.high = i'.high} and they have got the bounds excluded/included.
     *
     * @param obj object to which this interval is to be compared
     * @return {@code true} if and only if the specified {@code Object} is a {@code Interval} whose low and high are equal to this {@code Interval's}.
    ...
    public String toString(boolean timesAsDoubles) {
        ...
        if (timesAsDoubles) {
            ...
            sb.append(AttributeUtils.getXMLDateStringFromDouble(low));
            sb.append(", ");
            sb.append(AttributeUtils.getXMLDateStringFromDouble(high));
        }
        if (value != null) {
            sb.append(", ");
    ...
    /**
     * Returns a string representation of this interval in one of the formats: <ol> <li> {@code [low, high, value]} <li> {@code (low, high, value]} <li> {@code [low, high, value)} <li>
     * {@code (low, high, value)} </ol>
     *
