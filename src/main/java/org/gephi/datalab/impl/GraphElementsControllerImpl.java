...
/**
 * Implementation of the GraphElementsController interface declared in the Data
 * Laboratory API
 *
...
public class GraphElementsControllerImpl implements GraphElementsController {
    ...
    public Node duplicateNode(Node node) {
        ...
        Node copy = copyNodeRecursively(node, hg.getParent(node), hg);// Add
        // copy
        // to
        // the
        // same
    ...
    public Edge createEdge(Node source, Node target, boolean directed) {
        ...
        if (directed) {
            ...
            if (getDirectedGraph().addEdge(newEdge)) {// The edge will be
                // created if it does
                // not already exist.
                return newEdge;
            } else {
            ...
            if (getUndirectedGraph().addEdge(newEdge)) {// The edge will be
                // created if it does
                // not already exist.
                return newEdge;
            } else {
    ...
    public void deleteEdgeWithNodes(Edge edge, boolean deleteSource, boolean deleteTarget) {
        ...
        removeEdge(edge, getGraph());// If no node is deleted, we need to remove
        // the edge.
    }
    ...
    public boolean groupNodes(Node[] nodes) {
        ...
        // group.getNodeData().setLabel(NbBundle.getMessage(GraphElementsControllerImpl.class,
        // "Group.nodeCount.label", nodes.length));
        ...
        // } catch (Exception e) {
        // graph.readUnlockAll();
        // NotifyDescriptor.Message nd = new
    ...
    public boolean ungroupNodeRecursively(Node node) {
        if (canUngroupNode(node)) {
            HierarchicalGraph hg = getHierarchicalGraph();
            // We can get directly all descendant nodes withoud using recursion
            // and break the groups:
    ...
    public Node mergeNodes(Node[] nodes, Node selectedNode, AttributeRowsMergeStrategy[] mergeStrategies, boolean deleteMergedNodes) {
        ...
        // Set properties (position, size and color) using the selected node
        // properties:
            ...
            for (Edge edge : getNodeEdges(node)) {
                if (edge.getSource() == node) {
                    if (nodesSet.contains(edge.getTarget())) {
                        newEdge = createEdge(newNode, newNode, edge.isDirected());// Self
                        // loop
                        // because
                        // of
                        // edge
    ...
    public Node[] getAvailableGroupsToMoveNodes(Node[] nodes) {
        if (canGroupNodes(nodes)) {
            ...
            // All have the same parent, get children and check what of them are
            // groups and are not in the nodes array:
            Node parent = hg.getParent(nodes[0]);
            Node[] possibleGroups;
    ...
    public boolean removeNodeFromGroup(Node node) {
        if (isNodeInGroup(node)) {
            ...
            hg.readUnlock();
            if (childrenCount == 1) {
                hg.ungroupNodes(parent);// Break group when the last child is
                // removed.
            } else {
